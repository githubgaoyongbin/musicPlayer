<template>
  <div class="result-container" :key="key">
...
<script>
  ...
  export default {
    ...
    // 生命周期钩子 回调函数
    async created() {
      await this.loadSongData(this.$route.query.q)
    },
    // 侦听器
    watch: {
      async "$route.query.q"(newVal,oldVal){
        if(newVal !=oldVal){
         await this.loadSongData(newVal)
         this.key = !this.key
        }
      },
      activeIndex() {
    ...
    methods: {
      ...
      async loadSongData(q){
        ...
        console.log("res",res.data.result.songs)
        this.songList = res.data.result.songs
        // 计算歌曲时间
        for (let i = 0; i < this.songList.length; i++) {
          let min = parseInt(this.songList[i].duration / 1000 / 60)
